...
def sinario(user_id):
    ...
    reservation_id = "37"
    url = "http://localhost:8084/v1/reservation-products/" + reservation_id + "/stock"
    try:
        response = requests.get(url)
        print(f"재고 수량 - {response.json()['data']['stockCount']}")
    ...
    url = "http://localhost:8084/v1/orders"
    response = None
    try:
        body = {
            "productId" : 37, 
        ...
        print(f"주문(결제화면들어가기) {response.json()['desc']}")
    ...
    if random_number < 20:
        url = "http://localhost:8084/v1/orders/" + str(order_id)
        try:
    ...
    url = "http://localhost:8084/v1/payments"
    try:
        body = {
            "orderId" : order_id,
            "memberId" : user_id
